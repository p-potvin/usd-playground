...
- `da3-draft`/`da3-incremental` (`--gs-only`, direct 3DGS) had **never run once** before 2026-07-30 — three stacked failures (silent Dockerfile fallback → no nvcc in the nerfstudio base → `e3nn` ImportError swallowed by DA3 and resurfacing as a `NameError` mid-inference). All fixed; image `vw-studio-da3-gs` now builds green. Full write-up incl. the abandoned local-Windows-build side quest and the ZeroGPU finding: [docs/da3-direct-3dgs-debugging-20260730.md](docs/da3-direct-3dgs-debugging-20260730.md). **Still needs one green end-to-end run** — blocked on HF GPU capacity, resume with `tools/retry_da3_draft_recon.py local-run-20260730-105436`. Repoint both presets at `vw-studio-da3` (drop the `-gs` override in `presets.py`) once that lands.
